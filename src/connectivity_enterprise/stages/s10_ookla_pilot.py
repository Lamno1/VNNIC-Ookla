...
def run(ctx):
    ...
    ookla_change_rank=oc.rank(); vnnic_change_rank=vc.rank(); change_rank_rho,change_rank_p=spearmanr(vc,oc)
    n_ci=len(oc); z_ci=np.arctanh(change_rank_rho); se_ci=1/np.sqrt(n_ci-3); ci_lo,ci_hi=np.tanh(z_ci-1.96*se_ci),np.tanh(z_ci+1.96*se_ci)
    changedata=pd.DataFrame({"province_id_legacy63":oc.index,"ookla_change":oc.values,"vnnic_change":vc.values,"ookla_change_rank":ookla_change_rank.values,"vnnic_change_rank":vnnic_change_rank.values}); p=WORKING_ROOT/"artifacts/figures/figure_11_measurement_change_comparison_data.csv"; changedata.to_csv(p,index=False,encoding="utf-8-sig"); figouts.append(p)
    fig,axes=plt.subplots(1,2,figsize=(13,6))
    axes[0].scatter(vc,oc,alpha=.7); axes[0].axhline(0,color="#777"); axes[0].axvline(0,color="#777"); axes[0].set(xlabel="VNNIC Q2 change (Mbps)",ylabel="Ookla Q2 change (Mbps)",title="(a) Raw change, 2021–2024")
    axes[1].scatter(vnnic_change_rank,ookla_change_rank,alpha=.7); axes[1].plot([1,63],[1,63],color="#555",linestyle="--"); axes[1].set(xlabel="VNNIC change rank",ylabel="Ookla change rank",title="(b) Change-rank comparison")
    axes[1].annotate(f"Spearman $\\rho$={change_rank_rho:.3f}\n95% CI [{ci_lo:.3f}, {ci_hi:.3f}]\n$p$={change_rank_p:.3f}",xy=(.05,.95),xycoords="axes fraction",va="top",fontsize=10,bbox=dict(boxstyle="round",facecolor="white",edgecolor="#999"))
    fig.suptitle("Connectivity change comparison, 2021–2024")
    figouts+=savefig(fig,WORKING_ROOT/"artifacts/figures/figure_11_measurement_change_comparison")
